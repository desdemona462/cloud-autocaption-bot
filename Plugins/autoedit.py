...
async def editing(bot, message):
      ...
      if (message.document or message.video or message.audio): 
          ...
          new_file = "[@ULLASTV]- " + file.replace("-", " ").replace(".", " ").replace("@", " ").replace("_", " ").replace("avi", " ").replace("mp4", " ").replace("mkv", " ").replace("pdf", " ").replace("apk", " ").replace("mp3", " ").replace("zip", " ")
          New_name = f"**📑 File Name:** `{new_file}`"
